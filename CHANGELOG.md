...
- **Schedule times are a product surface: `plainkeep job set`, and the setup wizard asks.** The
  ...
  the registry lacks is **seeded from the engine defaults** — which is how any job reaches a vault at
...
- **A malformed jobs registry is now refused, and reported, everywhere.** Two shapes reach the
  ...
  vault) and a job **value** that is not an object. Both are refused when the registry is read,
  which is upstream of every action — so `plainkeep job disable`, deliberately permissive about risk
  class, can no longer be handed a key that resolves outside `~/Library/LaunchAgents`, and no action
  throws where the product promises a diagnosis. `plainkeep doctor` reports the refusal as a warning
  naming the offending entry instead of passing the check vacuously, and the `automation` setup layer
  reports `blocked` with the repair rather than offering a command that would refuse.
